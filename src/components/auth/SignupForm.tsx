...
export const SignupForm: React.FC<SignupFormProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (requireTermsAcceptance && !acceptTerms) {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (result.success) {
        ...
        onError?.(result.error || 'Google signup failed');
      }
    } catch (error) {
  ...
  // Clear errors when switching signup types
  useEffect(() => {
    clearError();
    setFormErrors({});
  }, [signupType, clearError]);
    ...
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder={signupType === 'email' ? 'john@example.com' : '+1 (555) 123-4567'}
          required
          disabled={state.isLoading}
          className={`h-11 ${formErrors[signupType] ? 'border-red-500' : ''}`}
        />
        {formErrors[signupType] && (
          <p className="text-sm text-red-500">{formErrors[signupType]}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <Input
            ...
            required
            disabled={state.isLoading}
            className={`h-11 pr-10 ${formErrors.confirmPassword || !passwordsMatch ? 'border-red-500' : ''}`}
          />
          <Button
            type="button"
            variant="ghost"
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            <Input
              ...
              required
              disabled={state.isLoading}
              className={`h-11 ${formErrors.captcha ? 'border-red-500' : ''}`}
            />
            <Button
              type="button"
              variant="outline"
          ...
          {formErrors.captcha && (
            <p className="text-sm text-red-500">{formErrors.captcha}</p>
          )}
        </div>
      )}
      ...
      <Button
        ...
        disabled={state.isLoading || !passwordStrength.isValid || !passwordsMatch}
      >
        {state.isLoading ? (
          <>
